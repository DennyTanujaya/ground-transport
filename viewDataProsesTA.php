    ...
    <tbody>
        <?php
            ...
            if(isset($keyword)){ // Jika veriabel $keyword ada (user telah mengklik tombol search)
                $param = $keyword;
                $sql = $pdo->prepare("SELECT * FROM operation WHERE service = 'TA' AND slStatus = 'OK' AND serviceDate = :se OR clientName = :na OR bookingReference = :tp OR supplier = :su OR agent = :ag OR activities = :mo ORDER BY serviceDate DESC, agent ASC");
            ...
            {
                $sql = $pdo->prepare("SELECT * FROM operation WHERE service = 'TA' AND slStatus = 'OK' ORDER BY serviceDate DESC, agent ASC");
                $sql->execute();
            }
