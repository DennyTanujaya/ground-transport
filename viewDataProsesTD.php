    ...
    <tbody>
        <?php
            ...
            if(isset($keyword)){ // Jika veriabel $keyword ada (user telah mengklik tombol search)
                $param = $keyword;
                $sql = $pdo->prepare("SELECT * FROM operation WHERE service = 'TD' AND slStatus = 'OK' AND serviceDate = :se OR clientName = :na OR bookingReference = :br OR supplier = :su OR agent = :ag OR activities = :mo ORDER BY serviceDate DESC, agent ASC");
                $sql->bindParam(':se', $param);
                $sql->bindParam(':br', $param);
            ...
            {
                $sql = $pdo->prepare("SELECT * FROM operation WHERE service = 'TD' AND slStatus = 'OK' ORDER BY serviceDate DESC, agent ASC");
                $sql->execute();
            }
